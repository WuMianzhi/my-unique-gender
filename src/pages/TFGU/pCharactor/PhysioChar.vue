<template>
  <div class="wrapper rounded-2xl">
    <h1 class="text-4xl main-title">{{ $t("genderedBiology.title") }}</h1>
    <div class="flex justify-center flex-col p-4">
      <div class="flex justify-between subtitle">
        <p>{{ $t("genderedBiology.vagina") }}</p>
        <p>{{ $t("genderedBiology.undifferentiated") }}</p>
        <p>{{ $t("genderedBiology.penis") }}</p>
      </div>
      <div class="py-3">
        <input
          type="range"
          name=""
          id=""
          class="w-full ring-4 ring-white/30 rounded-full h-5 rainbow-bg box-content"
        />
      </div>
              ...
              <button
                ...
                :style="{
                  'background-color': calInterColorByHex(
                    hormone.colorSeries[0],
                    hormone.colorSeries[1],
                    i
                  ),
                }"
                :class="[
                  'score-btn',
                  i * 5 == hormone.value ? 'relative bottom-2' : '',
                ]"
              ></button>
            </template>
          </div>
...
const hormones = reactive([
  {
    name: setupI18n.global.t("genderedBiology.estrogen"),
    colorSeries: ["ff9292", "ffd6a7"],
    value: 50,
    pic: estrogen,
  },
  {
    name: setupI18n.global.t("genderedBiology.androgen"),
    colorSeries: ["ffd6a7", "fffb8f"],
    value: 50,
    pic: androgens,
  },
  {
    name: setupI18n.global.t("genderedBiology.progesterone"),
...
input::-webkit-slider-thumb {
  ...
  background-image: linear-gradient(
      140.55deg,
      rgba(216, 235, 255, 0.2) 1.7%,
      rgba(255, 234, 253, 0.199136) 44%,
      rgba(254, 255, 218, 0.1979) 103.46%
    ),
    radial-gradient(
      50% 50% at 50% 50%,
      #ffffff 43.23%,
